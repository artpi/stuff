...
- FR-009A: Before offering creation as the primary action, the app must discover app-authorized stuff inventory Sheets available under the `drive.file` scope, offer to continue with them in most-recently-modified order, and make creating another inventory an explicit secondary action. Failed initialization must move the partial dedicated root to Drive trash when possible.
